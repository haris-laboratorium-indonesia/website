...
import { ExternalLink, InternalLink, Title } from '@/components/Materi';
...
export default function SiteMap() {
  ...
  return (
    <Layout browserTitle='Site Map' description='Site Map'>
      <Title name='Site Map' />
      <div className='mt-10 border-t border-gray-500 border-dashed'>
        <BiggerThanTopic name='General Information' />
        <div className='grid grid-cols-2 sm:gap-5 sm:grid-cols-3 lg:grid-cols-4'>
...
const Topic = ({ name }) => {
  return <h3 className='mt-10 text-lg font-semibold text-gray-700'>{name}</h3>;
};

const BiggerThanTopic = ({ name }) => {
  return <h3 className='-mb-5 text-2xl font-bold text-gray-700 mt-7'>{name}</h3>;
};
