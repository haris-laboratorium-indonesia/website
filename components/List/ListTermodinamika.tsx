...
const ListTermodinamika = ({ title, materi, page }) => {
  const H1 = 'block text-gray-200 hover:bg-gray-600 hover:text-white rounded-md px-2 py-1';
  const H2 = 'block text-gray-200 hover:bg-gray-600 hover:text-white rounded-md pl-6 pr-2 py-1';

  return (
    <Menu as="div" className="sticky z-40 flex flex-col w-full my-5 top-2 md:w-1/2 sm:mx-auto">
      <div className="flex items-center justify-between p-1 bg-gray-800 rounded-lg shadow-xl">
        <Link href={materi}>
        ...
        <Link href={`/fisika/${page}`}>
          <a className="font-medium tracking-wider text-gray-100 hover:text-white">{title}</a>
        </Link>
