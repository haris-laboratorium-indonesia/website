...
export default function BottomNav() {
  return (
    <div className='sticky bottom-0 block sm:hidden'>
      <div className='grid grid-cols-5 bg-white border border-t border-gray-200 drop-shadow-xl'>
        {BottomNavData.map(a => (
          <Nav key={a.title} to={a.to} svg={a.svg} title={a.title} />
        ))}
      </div>
    </div>
  );
...
const Nav = ({ to, svg, title }) => {
  return (
    <Link href={to}>
      <a className='flex flex-col items-center justify-center text-center hover:bg-gray-100 pb-1 pt-1.5'>
        <svg
...
const BottomNavData = [
  ...
  { to: '/fisika', svg: 'M13 10V3L4 14h7v7l9-11h-7z', title: 'Fisika' },
  {
    to: '/kalkulator',
