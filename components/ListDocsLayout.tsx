...
export default function ListDocsLayout({ branch, to, title, children }) {
  ...
  const titleStyle =
    'block w-1/2 md:w-full py-2.5 font-semibold text-center text-gray-800 sm:pl-5 sm:text-left text-tiny ';
  const MenuItemsStyle =
    ' w-full p-2 space-y-2 overflow-y-auto  rounded-md shadow-lg bg-white max-h-96';
  return (
    <div className='sticky top-0 z-40 '>
      <Menu as='div' className='flex flex-col w-full mb-5 sm:mb-2 sm:mx-auto '>
        {({ open }) => (
          <>
            <section className={`${open ? 'rounded-b-none' : ''} ${sectionStyle} `}>
              <Link href={`/${branch}`}>
              ...
              <Link href={`/${branch}/${to}`}>
                <a className={titleStyle}>{title}</a>
              </Link>
            ...
            <Menu.Items className='flex justify-center w-full '>
              <div className={`${open ? 'rounded-t-none' : ''} ${MenuItemsStyle} `}>{children}</div>
            </Menu.Items>
          </>
        )}
      </Menu>
