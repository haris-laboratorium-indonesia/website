...
const TabPhy = ({page, to}) => {
  return (
    <header className="z-40 flex flex-row items-center justify-between w-full h-auto my-3 bg-white ">
      ...
      <section className="flex flex-row items-center justify-end p-0.5 space-x-0.5 text-sm sm:text-base bg-gray-100 border border-gray-300 rounded-lg ">
        <Link href={to}>
          <a>
            <div className="px-2 py-1 border border-gray-100 rounded-md ">
              Matematika
            </div>
          </a>
        </Link>
        <div className="px-2 py-1 bg-white border border-gray-200 rounded-md shadow-md text-harislab">
          Fisika
        </div>
      </section>
    </header>
